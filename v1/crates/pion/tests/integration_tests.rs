...
fn main() {
    ...
    let tests = std::iter::empty()
        .chain(find_source_files("tests/elab").map(|path| elab_test(&path, update_snapshots)))
        .collect();
...
fn elab_test(input_path: &Path, update: bool) -> libtest_mimic::Trial {
    let test_name = input_path.strip_prefix("tests").unwrap();
    ...
    libtest_mimic::Trial::test(test_name, move || {
        ...
        command.args(["--dump-core", "check", &input_path.display().to_string()]);
